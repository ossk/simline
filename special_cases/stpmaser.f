...
c     ***********************************************************
      ...
      real phi,prof,tau1,tau2,tau,tauq,rel
      real g1,ex1,ex2
      real ihlp1,ihlp2
...
c     be avoided
c     ******************************************************
      if (abs(cp(l)*prof).lt.explin) then
c     if (cp(l)*prof.lt.explin) then
      do 650 j=jj1,jj2
      tau1 = z*cap(j,l)
      cap(j,l) = cp(l)*pp(j)
      tau2 = z*cap(j,l)
      tauq = tau1*tau2
      iplus(j,l) = (iplus(j,l)+sc(l)*tau2+ss(l)*tau1
     %	 +0.66667*tauq*(2.0*sc(l)+ss(l)))/(1.0+tau1+tau2+2.0*tauq)
650   continue
      else
c     ******************************************************
...
c     if (tau.lt.explin) then
c     Thin case - linear approx.
      tauq = tau1*tau2
      iplus(j,l) = (iplus(j,l)+sc(l)*tau2+ss(l)*tau1
     %	 +0.66667*tauq*(2.0*sc(l)+ss(l)))/(1.0+tau+2.0*tauq)
      else if (rel.lt.softzone) then
c     else if (tau.lt.softzone) then
c     Small transition regime - superposition
      tauq = tau1*tau2
      ihlp1 = (iplus(j,l)+sc(l)*tau2+ss(l)*tau1
     %	 +0.66667*tauq*(2.0*sc(l)+ss(l)))/(1.0+tau+2.0*tauq)
     ...
     %              (g1+ex1*ex1*(iplus(j,l)-ss(l))))
      iplus(j,l)=(ihlp1*(softzone-rel)+ihlp2*(rel-explin))/
     %              (softzone-explin)
c     iplus(j,l)=(ihlp1*(softzone-tau)+ihlp2*(tau-explin))/
c     %              (softzone-explin)
      else
