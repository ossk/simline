...
c     **************************************************************
      ...
      real zz(nstretch),uf(nstretch),iff(nstretch)
      real plow,pup,sigsqu,dintlin,specerr
...
150   continue
      iconv(j)=dintlin(zz,iff,imin,imax)
100   continue
      return
      end
...
c     initialize profile parameters
      do 100 j=j1,j2
100   prof(j)=phi(j*dny,vs,sig)
c     Treat the different lines
      do 200 l=1,lin
      ii=0
      do 250 j=j1,j2
      ii=ii+1
250   ff(ii)=iplus(j,l)*prof(j)
